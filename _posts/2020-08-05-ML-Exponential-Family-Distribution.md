...
2. 共轭。我们通过既得样本求后验：
   ...
   给出性质：若先验与似然共轭，则先验与后验相同。如令P(x|z)~二项分布，P(z)~Beta，则P(z|x)~Beta。通过共轭，我们可以非常取巧的避免求积分项，从而直接得到结果。

3. 最大熵。指数族分布满足最大熵思想（无信息先验），也就是说，对于经验分布，利用最大熵原理推导出的分布就是指数族分布。这一点会在下面的最大熵思想部分详细给出。

【三大算法模型】

1. 广义线性模型。根据线性模型定义$$y=f(w^Tx)$$，我们很容易可以窥见指数族分布与线性模型的关系：对于用于不同用途的线性模型，有$$y|x~ExpFamily Dist$$，eg. 对于线性回归问题，有$$y|x~N(\mu,\epsilon)$$。
